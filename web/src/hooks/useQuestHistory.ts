 ...
 * Quest History Hook
 * 
 * Fetches and manages quest history data.
 */
...
type QuestStatus = 'completed' | 'partial' | 'missed'
...
interface UseQuestHistoryReturn {
  history: QuestHistoryEntry[]
  stats: QuestHistoryStats
  isLoading: boolean
  error: string | null
...
export function useQuestHistory({
  dateRange,
  filter,
}: UseQuestHistoryOptions): UseQuestHistoryReturn {
  const [rawHistory, setRawHistory] = useState<QuestHistoryEntry[]>([])
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/quests/history?days=${days}`)
      ...
      setRawHistory(generateSampleHistory(dateRange))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  for (let d = 0; d < days; d++) {
    ...
    for (let q = 0; q < questCount; q++) {
      ...
      const status: QuestStatus = 
        Math.random() > 0.2 ? 'completed' :
        Math.random() > 0.5 ? 'partial' : 'missed'
      ...
      history.push({
        ...
        progress: status === 'completed' ? 100 : status === 'partial' ? 50 + Math.random() * 40 : Math.random() * 30,
        target: 100,
      })
    }
  }
